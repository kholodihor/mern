...
const ReviewCard = ({ data: card, index }: { data: any; index?: number }) => {
  return (
    <article
      id={card.name}
      className="mx-auto flex  h-[315px] overflow-y-auto no-scrollbar w-[90%] flex-col items-center justify-center rounded-[2.5rem] border border-white px-4 py-6 sm:w-[75%] md:w-[300px]"
    >
      <div className="flex h-full w-full flex-col justify-start gap-2">
        ...
        <p className="text-gray-400 py-2">{card.review}</p>
      </div>
    </article>
  );
};
