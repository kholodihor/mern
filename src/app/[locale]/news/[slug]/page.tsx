...
import { PageMetadata, INewsArticle } from "@/types";
...
export async function generateMetadata({
  ...
  const title = articleData && articleData.title && articleData.title[locale] ? 
    `${articleData.title[locale]} | ${localeMetadata.title}` : 
    `${localeMetadata.title} | ${slug}`;
    
  const description = articleData && articleData.short_text && articleData.short_text[locale] ? 
  ...
  return {
    ...
    openGraph: {
      ...
      title: title,
      description: description,
      images: articleData && articleData.images && articleData.images.length > 0 ? 
        [{ url: articleData.images[0], width: 1200, height: 630, alt: articleData.title[locale] || 'News article' }] : 
