...
const processDocumentImages = async (
  ...
  console.log(`Processing ${images.length} images for ${collectionName}/${docId}`);
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    const newUrl = await processImage(imageUrl, i, collectionName, docId, tempDir);
  ...
  try {
    ...
    for (const galleryDoc of gallerySnapshot.docs) {
      ...
      if (galleryData.images && galleryData.images.length > 0) {
        ...
        const newImages = await processDocumentImages('gallery', galleryDoc.id, galleryData.images, tempDir);
    ...
    for (const newsDoc of newsSnapshot.docs) {
      ...
      if (newsData.images && newsData.images.length > 0) {
        ...
        const newImages = await processDocumentImages('news', newsDoc.id, newsData.images, tempDir);
